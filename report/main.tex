...
Random Forest achieved an $R^2$ of \textbf{0.9848} and MAE of \textbf{3,953.63 hg/ha},
...
\rowcolor{lightgrey}
Random Forest        & \textbf{3,953.63} & \textbf{10,409.49} & \textbf{0.9848} \\
\midrule
\rowcolor{green!8}
...
\begin{axis}[
  ...
  \addplot[fill=primary,draw=primary] coordinates
    {(MAE,3954)(RMSE,10409)({$R^2\!\times\!10^4$},9848)};
...
The train $R^2$ of \textbf{0.9978} vs test $R^2$ of \textbf{0.9848} gives a
...
\begin{tikzpicture}[xscale=11, yscale=0.52]
  \draw[gray!20,dashed](0.1,0.2)--(0.1,14.8);
  \draw[gray!20,dashed](0.2,0.2)--(0.2,14.8);
  \draw[gray!20,dashed](0.3,0.2)--(0.3,14.8);
  \draw[gray!20,dashed](0.4,0.2)--(0.4,14.8);
  \fill[accent!45] (0,0.55)  rectangle (0.0008,1.45);  \node[left,font=\tiny] at(0,1) {Sorghum};           \node[right,font=\tiny] at(0.0008,1) {0.001};
  \fill[accent!48] (0,1.55)  rectangle (0.0008,2.45);  \node[left,font=\tiny] at(0,2) {Wheat};             \node[right,font=\tiny] at(0.0008,2) {0.001};
  \fill[accent!51] (0,2.55)  rectangle (0.0022,3.45);  \node[left,font=\tiny] at(0,3) {Soybeans};          \node[right,font=\tiny] at(0.0022,3) {0.002};
  \fill[accent!54] (0,3.55)  rectangle (0.0036,4.45);  \node[left,font=\tiny] at(0,4) {Maize};             \node[right,font=\tiny] at(0.0036,4) {0.004};
  \fill[accent!58] (0,4.55)  rectangle (0.0274,5.45);  \node[left,font=\tiny] at(0,5) {Yams};              \node[right,font=\tiny] at(0.0274,5) {0.027};
  \fill[accent!62] (0,5.55)  rectangle (0.0303,6.45);  \node[left,font=\tiny] at(0,6) {Rice/Paddy};        \node[right,font=\tiny] at(0.0303,6) {0.030};
  \fill[accent!66] (0,6.55)  rectangle (0.0394,7.45);  \node[left,font=\tiny] at(0,7) {Year};              \node[right,font=\tiny] at(0.0394,7) {0.039};
  \fill[accent!72] (0,7.55)  rectangle (0.0451,8.45);  \node[left,font=\tiny] at(0,8) {Plantains};         \node[right,font=\tiny] at(0.0451,8) {0.045};
  \fill[accent!78] (0,8.55)  rectangle (0.0877,9.45);  \node[left,font=\tiny] at(0,9) {Sweet Potatoes};    \node[right,font=\tiny] at(0.0877,9) {0.088};
  \fill[accent!84] (0,9.55)  rectangle (0.0945,10.45); \node[left,font=\tiny] at(0,10){Cassava};           \node[right,font=\tiny] at(0.0945,10){0.095};
  \fill[accent!90] (0,10.55) rectangle (0.1155,11.45); \node[left,font=\tiny] at(0,11){Rainfall};          \node[right,font=\tiny] at(0.1155,11){0.116};
  \fill[accent]    (0,11.55) rectangle (0.1170,12.45); \node[left,font=\tiny] at(0,12){Pesticides};        \node[right,font=\tiny] at(0.1170,12){0.117};
  \fill[primary!70](0,12.55) rectangle (0.1223,13.45); \node[left,font=\tiny] at(0,13){Avg Temp};          \node[right,font=\tiny] at(0.1223,13){0.122};
  \fill[primary]   (0,13.55) rectangle (0.3756,14.45); \node[left,font=\tiny] at(0,14){Potatoes};          \node[right,font=\tiny] at(0.3756,14){0.376};
  \draw[darkgrey,thick](0,0.2)--(0,14.8);
...
\end{tikzpicture}
\caption{Feature importance for the Random Forest Regressor (exact values from model).}
\end{figure}

\begin{insightbox}[Key Finding]
\textbf{Crop type} (especially Potatoes, 37.6\% importance) is the single most
dominant predictor. Environmental features --- avg.\ temperature (12.2\%), pesticides
(11.7\%), and rainfall (11.6\%) --- collectively account for significant importance.
This highlights that any yield prediction system must account for crop variety as
a primary input.
\end{insightbox}
...
\section{Phase 2: AI-Assisted Agricultural Advisory System}

\subsection{Introduction}

Phase~2 extends the CropCast deployment from yield prediction alone to a
\emph{hybrid} platform that combines the existing Random Forest regressor with
\emph{LangGraph} orchestration, \emph{retrieval-augmented generation} (RAG) over
local agronomy text, and a \emph{multi-turn conversational} interface powered by
the Anthropic API (official Python client in \texttt{agent/nodes.py}). The
structured advisory path (\texttt{run\_agent} in \texttt{agent/graph.py}) chains
parse $\rightarrow$ predict $\rightarrow$ risk assessment $\rightarrow$ document
retrieval $\rightarrow$ LLM synthesis. A separate compiled graph (\texttt{run\_chat})
runs only the \texttt{chat\_turn} node per message so conversation stays responsive
and does not re-execute the full pipeline every turn.

\subsection{Structured Advisory Pipeline}

Figure~\ref{fig:phase2-advisory-pipeline} shows the LangGraph workflow as
implemented in \texttt{build\_graph()}. The final artefacts are fields in
\texttt{FarmState} (e.g.\ \texttt{field\_summary}, \texttt{recommendations},
\texttt{sources}), not the PDF/HTML yield reports in \texttt{report\_generator.py},
which are generated on a separate ML-only path from the Streamlit UI.
...
\begin{tikzpicture}[
    ...
    box/.style={draw=primary, rounded corners=4pt, line width=0.6pt,
                align=center, minimum width=3.15cm, minimum height=1.05cm,
                font=\small, inner sep=5pt, fill=lightblue!35},
    arrow/.style={-{Stealth[length=2.4mm]}, thick, color=darkgrey}
]
\node[box] (input) {\textbf{User / default query}\\\footnotesize\texttt{parse\_input}};
\node[box, below=of input] (predict) {\textbf{Predict yield}\\\footnotesize\texttt{predict\_yield} (RF)};
\node[box, below=of predict] (risk) {\textbf{Risk vs benchmarks}\\\footnotesize\texttt{assess\_risk}};
\node[box, below=of risk] (rag) {\textbf{Retrieve chunks}\\\footnotesize\texttt{retrieve\_docs}};
\node[box, below=of rag] (llm) {\textbf{LLM + JSON}\\\footnotesize\texttt{generate\_advice}};
\node[box, below=of llm, fill=accent!15] (out) {\textbf{\texttt{FarmState}}\\\footnotesize summary, recommendations, sources};
...
\caption{Phase~2 \textbf{structured advisory} pipeline (\texttt{run\_agent} / \texttt{build\_graph} in \texttt{agent/graph.py}; node logic in \texttt{agent/nodes.py}). Yield PDF/HTML reports (\texttt{report\_generator.py}) are produced separately from the ML prediction flow.}
\label{fig:phase2-advisory-pipeline}
\end{figure}

\subsection{RAG Workflow}

Documents under \texttt{rag/docs/} are chunked and embedded with
\texttt{sentence-transformers/all-MiniLM-L6-v2}, then indexed in FAISS
(\texttt{rag/ingest.py}, persisted under \texttt{rag/vectorstore/}). At query
time, \texttt{rag/retriever.py} loads the index and runs
\texttt{similarity\_search}; retrieved text is passed into
\texttt{generate\_advice} and \texttt{chat\_turn}. Figure~\ref{fig:phase2-rag}
summarises this split between offline index construction and online retrieval.

\begin{figure}[H]
\centering
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.25cm and 2.1cm,
    box/.style={draw=primary, rounded corners=4pt, line width=0.55pt,
                align=center, minimum width=2.75cm, minimum height=0.95cm,
                font=\small, inner sep=4pt, fill=lightgrey},
    arrow/.style={-{Stealth[length=2.2mm]}, thick, color=darkgrey},
    lbl/.style={font=\footnotesize\itshape, text=darkgrey}
]

\node[box] (docs) {Corpus\\\texttt{rag/docs/*.txt}};
\node[box, right=of docs] (faiss) {FAISS index\\\texttt{vectorstore/}};
\node[box, below=1.6cm of docs] (q) {User query string};
\node[box, right=of q] (ret) {Similarity search\\\texttt{retriever.py}};
\node[box, right=of ret, fill=lightblue!40] (llm) {Claude\\\texttt{nodes.py}};

\draw[arrow] (docs) -- node[lbl, above, sloped, near start] {ingest} (faiss);
\draw[arrow] (q) -- (ret);
\draw[arrow] (faiss.south) -- ++(0,-0.45) -| (ret.north);
\draw[arrow] (ret) -- (llm);

\end{tikzpicture}%
}
\caption{RAG in the codebase: FAISS is built offline from embedded chunks; each advisory or chat turn runs similarity search (query embedded with the same MiniLM model) and passes top-$k$ chunks to the LLM.}
\label{fig:phase2-rag}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Phase~2 components mapped to the repository.}
\label{tab:phase2-components}
...
\color{white}\textbf{Role in codebase} \\
\midrule
\rowcolor{lightgrey}
Prediction engine &
scikit-learn Random Forest &
\texttt{predict\_yield} in \texttt{agent/nodes.py}; same artefacts in \texttt{streamlit\_app.py} for the UI \\
Advisory orchestration &
LangGraph &
\texttt{agent/graph.py} (\texttt{build\_graph}, \texttt{run\_agent}; \texttt{build\_chat\_graph}, \texttt{run\_chat}) \\
\rowcolor{lightgrey}
RAG &
FAISS + \texttt{all-MiniLM-L6-v2} &
\texttt{rag/ingest.py}, \texttt{rag/retriever.py} \\
LLM &
Anthropic Claude (Messages API) &
\texttt{anthropic} client in \texttt{agent/nodes.py}; prompts in \texttt{agent/prompts.py} \\
\rowcolor{lightgrey}
Frontend &
Streamlit &
\texttt{streamlit\_app.py} (multi-page router, advisory chat) \\
Reports &
ReportLab (PDF); HTML (Python template) &
\texttt{report\_generator.py} (\texttt{generate\_pdf\_report}, \texttt{build\_html\_report}) \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Advisory Pipeline Stages}

\begin{table}[H]
\centering
\caption{Structured advisory pipeline stages (\texttt{run\_agent} / \texttt{build\_graph}).}
\label{tab:phase2-pipeline-stages}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{L{2.6cm} X L{4.2cm}}
\toprule
\rowcolor{primary}
...
\rowcolor{lightgrey}
Parsing &
Defaults, clamps, query text &
\texttt{parse\_input} --- \texttt{agent/nodes.py} \\
Prediction &
RF inference &
\texttt{predict\_yield} --- \texttt{agent/nodes.py}; artefacts \texttt{model.pkl}, \texttt{scaler.pkl}, \texttt{features.pkl} \\
\rowcolor{lightgrey}
Risk analysis &
Ratio vs \texttt{CROP\_BENCHMARKS} &
\texttt{assess\_risk} --- \texttt{agent/nodes.py} \\
Retrieval &
Top-$k$ chunks &
\texttt{retrieve\_docs} $\rightarrow$ \texttt{rag/retriever.py}; edges in \texttt{agent/graph.py} \\
\rowcolor{lightgrey}
Advice generation &
Claude + JSON &
\texttt{generate\_advice} --- \texttt{agent/nodes.py} \\
Graph compile &
Pipeline wiring &
\texttt{build\_graph} --- \texttt{agent/graph.py} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Evolution from prediction-centric work (Phase~1) to the hybrid advisory system (Phase~2) in the same codebase.}
\label{tab:phase2-vs-phase1}
\renewcommand{\arraystretch}{1.22}
\begin{tabularx}{\textwidth}{L{2.9cm} X X}
\toprule
\rowcolor{primary}
\color{white}\textbf{Aspect} &
\color{white}\textbf{Phase 1 (emphasis)} &
\color{white}\textbf{Phase 2 (additions)} \\
\midrule
\rowcolor{lightgrey}
Primary focus &
Yield estimation from inputs &
Same RF prediction plus LangGraph advisory and RAG-backed chat \\
ML model &
Random Forest regressor &
Same serialized model and feature construction \\
\rowcolor{lightgrey}
Beyond classical ML &
--- &
Anthropic Claude + FAISS retrieval (\texttt{agent/}, \texttt{rag/}) \\
Interaction &
Form-driven prediction workflow &
Multi-turn chat and optional full-pipeline structured advisory \\
\rowcolor{lightgrey}
Outputs &
Predictions, charts, PDF/HTML yield reports &
Above plus LLM fields and chat replies (separate code paths from yield-only exports) \\
Layout &
Application-centric UI &
Modular \texttt{agent/} and \texttt{rag/}; main UI remains largely in \texttt{streamlit\_app.py} \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Risk and Quality Notation}

The agent's \texttt{assess\_risk} step compares predicted yield to static
benchmarks in \texttt{CROP\_BENCHMARKS} (\texttt{agent/nodes.py}). Separately, the
HTML report builder applies an absolute t/ha band for ``quality'' in
\texttt{build\_html\_report} (\texttt{report\_generator.py}). These two mechanisms
are related for communication but not identical functions.

\subsection{Representative Interface Views}

Figure~\ref{fig:phase2-ui} shows screens from the deployed Streamlit application:
the yield dashboard ties into farm context used by the advisory agent, and the
full report view illustrates PDF/HTML-style output from the prediction path.
The conversational \emph{Advisory Agent} tab is implemented in code but may not
appear in static screenshots.
...
\caption{Yield dashboard after inference (quality gauge and summary).}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{ui_report_full.png}
\caption{Downloadable yield analysis report (executive summary and KPIs).}
\end{subfigure}
\caption{Representative CropCast views supporting Phase~1--2 deployment; structured LLM advisory is delivered via a separate chat workflow in the same app.}
\label{fig:phase2-ui}
\end{figure}

\subsection{Evaluation and Robustness}

Held-out test performance for the Random Forest is summarised elsewhere in this
report (e.g.\ $R^2 \approx 0.98$ on the documented test split in the modelling
section). The advisory layer adds RAG-grounded prompts (\texttt{agent/prompts.py}),
model-ID fallbacks for Anthropic (\texttt{\_anthropic\_model\_candidates}), JSON
parse fallbacks in \texttt{generate\_advice}, and offline-style chat templates when
the API is unavailable, improving demo robustness without changing the sklearn
artefacts.

\subsection{Challenges and Limitations (Phase 2)}

\begin{itemize}[leftmargin=2em]
  \item Coupling deterministic inference with stochastic LLM outputs and strict
        JSON parsing for structured advice.
  \item Retrieval quality depends on chunking, query wording, and a static corpus
        unless \texttt{rag/docs/} is updated and the index rebuilt.
  \item Expert validation of natural-language recommendations is outside the scope
...
Several deliberate steps were taken to improve model performance and ensure
reliable generalisation:

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=2.5em]
  \item \textbf{Feature Scaling:} \texttt{StandardScaler} was applied to all
        numeric features before training. This ensures no single feature
        dominates due to scale differences, and improves convergence for
        Linear Regression.
  \item \textbf{Train/Test Split:} An 80/20 stratified split
        (\texttt{random\_state=42}) was used to ensure reproducibility and
        unbiased evaluation on held-out data.
  \item \textbf{Model Selection:} Random Forest was chosen over Linear
        Regression after EDA confirmed non-linear relationships and skewed
        distributions --- a deliberate algorithmic optimisation over a simpler
        baseline.
  \item \textbf{Hyperparameter Choice:} \texttt{n\_estimators=100} trees were
        used for the Random Forest, balancing predictive accuracy with
        computational cost. The generalisation gap of only \textbf{0.013}
        between train $R^2$ (0.9978) and test $R^2$ (0.9848) confirms the
        model is well-tuned without overfitting.
  \item \textbf{Outlier Retention:} Rather than removing outliers, they were
        retained as valid agricultural signals. Random Forest's robustness to
        outliers made this a safe and beneficial choice for model accuracy.
  \item \textbf{One-Hot Encoding:} Crop type was encoded into binary features
        rather than ordinal integers, preventing the model from assuming any
        false ordering between crop categories.
\end{enumerate}

\begin{insightbox}[Optimisation Result]
These combined steps reduced MAE by \textbf{87.5\%} compared to the Linear
Regression baseline --- from 31,711 hg/ha down to 3,953 hg/ha.
\end{insightbox}
...
on 28,242 records achieved an \textbf{$R^2$ of 0.9848} and \textbf{MAE of 3,953.63 hg/ha}
...
\begin{table}[H]
\centering
\caption{Breakdown of team responsibilities.}
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\textwidth}{L{3.5cm} L{3cm} X}
\toprule
\rowcolor{primary}
\color{white}\textbf{Member} &
\color{white}\textbf{Roll No.} &
\color{white}\textbf{Contribution} \\
\midrule
\rowcolor{lightgrey}
Divya Singh   & 2401020020 & Dataset collection, sourcing and preparation of all four FAO datasets \\
Khushi Batra  & 2401010226 & Report writing, data analysis, EDA insights and documentation \\
\rowcolor{lightgrey}
Shivam Mittal & 2401010443 & Streamlit UI development and hosting on Hugging Face Spaces \\
Aryan Vibhuti & 2401010105 & Model training, evaluation, feature engineering and optimisation \\
\bottomrule
\end{tabularx}
\end{table}
