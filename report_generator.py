...
class CropCastDocTemplate(SimpleDocTemplate):
    ...
    def build(self, flowables, onFirstPage=None, onLaterPages=None, canvasmaker=rl_canvas.Canvas):
        def _header_footer(canvas, doc):
            ...
            canvas.drawRightString(w - 10*mm, h - 19*mm, datetime.now().strftime("%B %d, %Y"))
            ...
            canvas.setFillColor(C_MUTED)
            canvas.drawString(10*mm, 4.5*mm, "Generated by CropCast · ML-powered agricultural yield intelligence")
            canvas.drawRightString(w - 10*mm, 4.5*mm, f"Page {doc.page}")

            canvas.restoreState()

        super().build(flowables, onFirstPage=_header_footer, onLaterPages=_header_footer,
...
STYLES = {
    "hero_num": S("hero_num", fontName="Helvetica-Bold", fontSize=38, textColor=C_ACCENT,
                  leading=42, spaceAfter=2, alignment=TA_CENTER),
    "hero_unit": S("hero_unit", fontSize=9, textColor=C_MUTED, leading=14, spaceAfter=2, alignment=TA_CENTER),
    "hero_sub":  S("hero_sub", fontSize=8, textColor=C_TEXT, leading=12, spaceAfter=0, alignment=TA_CENTER),
    "section_hd": S("section_hd", fontName="Helvetica-Bold", fontSize=7.5, textColor=C_ACCENT,
                     leading=10, spaceAfter=6, spaceBefore=6, letterSpacing=1.5),
    "body":      S("body", fontSize=8.5, textColor=C_TEXT, leading=14),
    "muted":     S("muted", fontSize=7.5, textColor=C_MUTED, leading=11),
    "label":     S("label", fontName="Helvetica-Bold", fontSize=7, textColor=C_MUTED,
                    leading=9, letterSpacing=0.8),
    "value":     S("value", fontName="Helvetica-Bold", fontSize=8.5, textColor=C_TEXT, leading=12),
    "note":      S("note", fontSize=7, textColor=C_MUTED, leading=10, spaceAfter=2),
    "footer_disc": S("footer_disc", fontSize=6.5, textColor=C_MUTED, leading=9, alignment=TA_CENTER),
}
...
def kv_table(rows, col_widths=(55*mm, 115*mm)):
    ...
    t.setStyle(TableStyle([
        ...
        ("ROWBACKGROUNDS", (0, 0), (-1, -1), [C_SURFACE, colors.HexColor("#141414")]),
...
def metric_row(metrics):
    ...
    for label, value, unit in metrics:
        cell_data.append([
            ...
            Paragraph(str(value), ParagraphStyle("mv", fontName="Helvetica-Bold",
                       fontSize=13, textColor=C_ACCENT, leading=16, alignment=TA_CENTER)),
            Paragraph(unit, ParagraphStyle("mu", fontSize=7, textColor=C_MUTED,
                       leading=9, alignment=TA_CENTER)),
        ])

    col_w = 170 * mm / len(metrics)
    rows = [[Table([[r[0]], [r[1]], [r[2]]], colWidths=[col_w - 4*mm]) for r in cell_data]]
...
def benchmark_table(predicted, reg_avg, glob_avg):
    ...
    rows = [header,
        [Paragraph("Your Prediction", STYLES["value"]),
         Paragraph(f"{predicted:,.0f}", ParagraphStyle("bv", fontName="Helvetica-Bold",
        ...
        [Paragraph("Regional Average", STYLES["body"]),
         ...
         Paragraph(f"{sign_r}{pct_reg:.1f}%",
                   ParagraphStyle("pct", fontSize=8.5, leading=12,
        ...
        [Paragraph("Global Average", STYLES["body"]),
         ...
         Paragraph(f"{sign_g}{pct_glob:.1f}%",
                   ParagraphStyle("pct2", fontSize=8.5, leading=12,
    ...
    t.setStyle(TableStyle([
        ...
        ("ROWBACKGROUNDS", (0, 1), (-1, -1), [C_SURFACE, colors.HexColor("#141414")]),
        ("LINEBELOW",   (0, 0), (-1, -2), 0.4, C_BORDER),
        ("TOPPADDING",  (0, 0), (-1, -1), 5),
        ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
        ("LEFTPADDING", (0, 0), (-1, -1), 7),
...
def generate_pdf_report(result: dict) -> bytes:
    ...
    story.append(hero_block(yield_hg, tonne_ha, pct_vs_avg,
                             r["item"], r["area"], r["year"]))
    story.append(Spacer(1, 4*mm))
...
def build_html_report(r: dict) -> str:
    ...
    if tonne_ha < 1.0:   band = "Poor"
    elif tonne_ha < 3.0: band = "Fair"
    elif tonne_ha < 6.0: band = "Good"
    elif tonne_ha < 12:  band = "High"
    ...
    RA = "#5BB336"; RA2 = "#7ED458"
    RB = "#080808"; RB2 = "#111111"; RT = "#E8E8E8"; RM = "#888888"; RBD = "#1F1F1F"

    rain_note = "Good rainfall for the selected crop." if rainfall >= 800 else "Below optimal — irrigation may be needed."
    temp_note = "Temperature within productive range." if 15 <= avg_temp <= 30 else "Temperature may limit yield potential."
    pest_note = "Input level appears moderate." if pesticides <= 200 else "High pesticide use — consider integrated pest management."
